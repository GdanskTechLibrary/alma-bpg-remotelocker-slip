...
export function _get_requested_resources(restService: CloudAppRestService, idents_ordered: Array<any>, idents_checked: Array<any>, circulation_desk: string, library: string, show_primary_id: boolean, show_barcode: boolean, show_fullname: boolean): Observable<any> 
{
    let letter_param = '';
        ...
        .pipe(
            ...
            mergeMap(user_requests => forkJoin(user_requests.map((request) => {
                    ...
                    if (isTBookRequest(request)) {
                        return __retrevie_replace_uid(request, restService, idents_ordered, idents_checked, show_primary_id, show_barcode, show_fullname);
                    }
                })
            )),
            map(users_cd_requ => users_cd_requ?.filter(notnull => notnull !== null)),
